...
func SetupRoutes(db *gorm.DB, redisClient *redis.Client, cfg *config.Config) *gin.Engine {
	...
	shopeeAuthController := controllers.NewShopeeAuthController(cfg, db, authService)
	...
	r.POST("/api/v1/balance/admin/shopee/auth/cfg", authController.GetByPartnerId)
	// Shopee 授权链接生成（方便前端/浏览器获取授权URL）
	r.POST("/api/v1/balance/admin/shopee/auth/url", shopeeAuthController.GenerateAuthURL)
	// Shopee 刷新令牌接口
	r.POST("/api/v1/balance/admin/shopee/auth/refresh", shopeeAuthController.RefreshToken)
