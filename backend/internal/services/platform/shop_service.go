...
func (s *ShopService) ListShops(ctx context.Context, ownerID int64, keyword, status string, page, pageSize int) ([]models.ShopWithAuth, int64, error) {
	...
	if status != "" {
		switch status {
		case "enabled":
			query = query.Where("status = ?", consts.ShopStatusEnabled)
		case "disabled":
			query = query.Where("status = ?", consts.ShopStatusDisabled)
		}
	}
...
func (s *ShopService) UpdateShopStatus(ctx context.Context, shopID int64, status int) error {
	result := s.db.Model(&models.Shop{}).Where("shop_id = ?", shopID).Update("status", status)
