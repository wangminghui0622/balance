...
func NewMaintenanceScheduler(logger ...*log.Logger) *MaintenanceScheduler {
	var l *log.Logger
...
func (s *MaintenanceScheduler) Start() {
	s.logger.Println("[Maintenance] 启动维护任务调度器...")
	...
	_, err := s.cron.AddFunc("0 0 2 * * *", func() {
		...
		if err != nil {
			s.logger.Printf("[Maintenance] 日志归档失败: %v", err)
		} else {
			s.logger.Printf("[Maintenance] 日志归档完成，归档 %d 条记录", count)
		}
	})
	if err != nil {
		s.logger.Printf("[Maintenance] 添加日志归档任务失败: %v", err)
	}
	...
	_, err = s.cron.AddFunc("0 0 3 * * *", func() {
		...
		if err := s.statsService.GenerateDailyStats(ctx); err != nil {
			s.logger.Printf("[Maintenance] 生成每日统计失败: %v", err)
		}
	})
	if err != nil {
		s.logger.Printf("[Maintenance] 添加每日统计任务失败: %v", err)
	}
	...
	_, err = s.cron.AddFunc("0 0 4 1 * *", func() {
		...
		if err != nil {
			s.logger.Printf("[Maintenance] 清理过期归档失败: %v", err)
		} else {
			s.logger.Printf("[Maintenance] 清理过期归档完成，删除 %d 条记录", count)
		}
	})
	if err != nil {
		s.logger.Printf("[Maintenance] 添加清理归档任务失败: %v", err)
	}
	...
	_, err = s.cron.AddFunc("0 */10 * * * *", func() {
		...
		if err != nil {
			s.logger.Printf("[Maintenance] 处理虾皮结算失败: %v", err)
		} else if count > 0 {
			s.logger.Printf("[Maintenance] 处理虾皮结算完成，结算 %d 笔订单", count)
		}
	})
	if err != nil {
		s.logger.Printf("[Maintenance] 添加虾皮结算任务失败: %v", err)
	}
	...
	_, err = s.cron.AddFunc("0 */10 * * * *", func() {
		...
		if err != nil {
			s.logger.Printf("[Maintenance] 处理虾皮调账失败: %v", err)
		} else if count > 0 {
			s.logger.Printf("[Maintenance] 处理虾皮调账完成，处理 %d 笔调账", count)
		}
	})
	if err != nil {
		s.logger.Printf("[Maintenance] 添加虾皮调账任务失败: %v", err)
	}

	s.cron.Start()
	s.logger.Println("[Maintenance] 维护任务调度器已启动")
...
func (s *MaintenanceScheduler) Stop() {
	s.logger.Println("[Maintenance] 停止维护任务调度器...")
	close(s.stopChan) // 通知后台任务停止
	s.cron.Stop()
	s.wg.Wait() // 等待所有后台任务结束
	s.logger.Println("[Maintenance] 维护任务调度器已停止")
}
...
func (s *MaintenanceScheduler) backfillStats() {
	...
	if err := s.statsService.db.Model(&struct{}{}).Table("platform_daily_stats").Count(&count).Error; err != nil {
		s.logger.Printf("[Maintenance] 检查统计数据失败: %v", err)
		return
	}

	if count > 0 {
		s.logger.Println("[Maintenance] 已有统计数据，跳过补充")
		return
	}

	s.logger.Println("[Maintenance] 开始补充历史统计数据...")
	...
	for i := 30; i >= 1; i-- {
		...
		if err := s.statsService.generateOrderDailyStats(ctx, statDate); err != nil {
			s.logger.Printf("[Maintenance] 补充 %s 订单统计失败: %v", statDate.Format("2006-01-02"), err)
		}
		if err := s.statsService.generateFinanceDailyStats(ctx, statDate); err != nil {
			s.logger.Printf("[Maintenance] 补充 %s 财务统计失败: %v", statDate.Format("2006-01-02"), err)
		}
		if err := s.statsService.generatePlatformDailyStats(ctx, statDate); err != nil {
			s.logger.Printf("[Maintenance] 补充 %s 平台统计失败: %v", statDate.Format("2006-01-02"), err)
		}
	}

	s.logger.Println("[Maintenance] 历史统计数据补充完成")
}
